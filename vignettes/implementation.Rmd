...
Here we will work with the original meuse dataset using only the x and y coordinates. The `design` element of the `nd_grts()` output now contains a `SpatialGridDataFrame` with the GRTS ranking. The `object` element is a `SpatialPointsDataFrame` containing the centroids from the `design` grid cells which contain at least one point from the input.
