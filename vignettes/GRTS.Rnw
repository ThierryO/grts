...
% \VignetteDepends{methods, plyr, ggplot2, xtable}
...
The next step is to split the four submatrices again in each four subsubmatrices (level 2). Like before we number them from 0 to 3 (table~\ref{tab:Level2_1D}). Next we prepend this number to the code of the submatrices (table~\ref{tab:Level2_1Dc}). Notice that when we sort the combined codes ascending, then first four codes indicate grid cells in each level 1 submatrix.
...
We keep repeating this procedure until all submatrices contain only one gridcell. In this case we have do it only once more, resulting in table~\ref{tab:Level3_1D} and \ref{tab:Level3_1Dc}. When requiring that the matrix is square and the number of rows is a power of two, the procedure will take the same amount of steps for each submatrix.
...
The work horse of the package is \texttt{QuadratRanking()} function. This function expectes to start with a zero-filled, square matrix and the number of rows is a power of 2. However the function does not do any checking on does assumptions. That would be a computational burden since the function is called recursively. Therefore one should not call \texttt{QuadratRanking()} directly but use the global wrapper function \texttt{GRTS()}. This wrapper function handles more conviniently different input formats.

\section{Getting a GRTS randomisation for a square matrix}

In this case we just supply the number of rows to the \texttt{GRTS()} function. Note that if the number of rows is not a power of 2, then \texttt{QuadratCount()} is run with next power of 2 as number of rows. Afterwards, the matrix is trimmed to contain the required number of rows.
<<echo = TRUE>>=
GRTS(8)
GRTS(7)
@
...
Typically one has the boundary of the studyarea as a GIS polygon. To accomodate this situation, \texttt{GRTS()} can handle objects of the class \texttt{SpatialPolygons}. Let's first create an object with a hypothetical studyarea consisting of 3 polygons: one island polygon and two adjacent polygons of which one contains a hole.
...
By default the grid starts a south-east corner (minimum of both coordinates) of the bounding box of the polygon object. When we specify \texttt{RandomStart = TRUE}, this origin is shifted at random in both direction between \texttt{0} and \texttt{cellsize} units.
